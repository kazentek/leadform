...
async function fireFacebookCAPI(payload, orderId, eventId) {
  ...
  // Use separate first_name / last_name when available (new form sends them)
  // Fall back to splitting customer_name for backward compatibility
  const rawFn = payload.first_name || (payload.customer_name || "").trim().split(/\s+/)[0] || "";
  const rawLn = payload.last_name  || (payload.customer_name || "").trim().split(/\s+/).slice(1).join(" ") || "";

  const hashedFn    = rawFn ? h(rawFn.toLowerCase()) : null;
  const hashedLn    = rawLn ? h(rawLn.toLowerCase()) : null;

  // Phone-based external_id (consistent with capi.js and mid-funnel events)
...
module.exports = async function handler(req, res) {
  ...
  const {
    variant_id, quantity,
    first_name, last_name, customer_name, // support both separate and combined
    phone, email,
  ...
  const combinedName = customer_name || `${first_name || ""} ${last_name || ""}`.trim();
  const firstName    = first_name  || combinedName.split(" ")[0] || combinedName;
  const lastName     = last_name   || combinedName.split(" ").slice(1).join(" ") || ".";

  if (!variant_id || !quantity || !phone || !wilaya || !commune || (!customer_name && !first_name)) {
  ...
  try {
    ...
    console.log(`[order] ✅ Created: ${order.name} — ${firstName} ${lastName} — ${wilaya}`);

    // Fire CAPI and await before responding
    const capiPayload = {
      ...body,
